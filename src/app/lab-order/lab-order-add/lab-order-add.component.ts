...
@Component({
  selector: 'app-lab-order-add',
  templateUrl: './lab-order-add.component.html',
  styleUrls: ['./lab-order-add.component.scss']
})
export class LabOrderAddComponent implements OnInit {
  ...
  addPatient() {
    ...
    this.dialog.open(PatientSearchComponent, dialogConfig).afterClosed().subscribe(res => {
      if (res) {
        ...
        this.orderData.patientId = res.patient.id;
      }
    })
  }
  ...
  onEnterFact(val: string) {
    if (this.orderData.customerId && val) {
      this.analysisOrderService.getData(`customers/${this.orderData.customerId}`).subscribe((customer: Customer) => {
        if (customer.cust_api_url) {
          const protocol = 'http://';
          const apiService = 'api';
          const port = 3800;
          const params = { user: '', password: '', invoice: val };
          this.analysisOrderService.getDataExternalApi(`invoice-info`, `${protocol}${customer.cust_api_url}:${port}/${apiService}`, params)
            .subscribe(
              (res) => { console.log(res) },
              (err) => { console.log(err) })
        }
