...
export class AnalysisResultListComponent {
    ...
    merge(this.sort.sortChange, this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          const urlString = `hos-cases/?done=${true}&page=${this.options.page}&size=${this.options.size}&search=${this.options.search}&orderBy=${this.options.orderBy}&orderDir=${this.options.orderDir}`;
  ...
  fetchAllOrders() {
    this.isLoadingResults = true;
    const urlString = `hos-cases/?done=${true}&page=${this.paginator.pageIndex}&size=${this.paginator.pageSize}&search=${this.options.search}&orderBy=${this.sort.active}&orderDir=${this.sort.direction}`;
  ...
  updateAnalysis(sample: LabSample) {
    // this.router.navigate(['/dashboard/updateanalysisresult', sample.id]);
    this.router.navigate(['/dashboard/updateanalysisresult'], { queryParams: { caseId: sample.id } });
  ...
  getData(): Observable<any> {
    this.isLoadingResults = true;
    // const urlString = `hos-cases/done/?done=${true}&page=${this.paginator.pageIndex}&size=${this.paginator.pageSize}&search=${this.options.search}&orderBy=${this.sort.active}&orderDir=${this.sort.direction}`;
    const urlString = `hos-cases/done/?page=${this.paginator.pageIndex}&size=${this.paginator.pageSize}&search=${this.options.search}&orderBy=${this.sort.active}&orderDir=${this.sort.direction}`;
    return this.repositoryService.getData(urlString).pipe(catchError(() => observableOf(null)));
  }
