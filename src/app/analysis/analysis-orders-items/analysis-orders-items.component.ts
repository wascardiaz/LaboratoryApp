...
@Component({
  selector: 'app-analysis-orders-items',
  templateUrl: './analysis-orders-items.component.html',
  styleUrls: ['./analysis-orders-items.component.scss']
})
export class AnalysisOrdersItemsComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AnalysisOrdersItemsComponent>,
  ...
  ngOnInit(): void {
    ...
    if ((this.data.item))
      this.formData = Object.assign({}, this.data.item);
    else
      this.formData = {
        id: 0,
        secuencia: 0,
  ...
  UpdatePrice(event: any) {
    if (event.selectedIndex == 0) {
      ...
      this.formData.cost = this.itemList[event.selectedIndex - 1].price;
