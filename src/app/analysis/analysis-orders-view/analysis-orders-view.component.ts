...
import { LabSample } from 'src/app/shared/models/lab-sample';
...
@Component({
  selector: 'app-analysis-orders-view',
  templateUrl: './analysis-orders-view.component.html',
  styleUrls: ['./analysis-orders-view.component.scss']
})
export class AnalysisOrdersViewComponent {
    ...
    merge(this.sort.sortChange, this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          const urlString = `hos-cases/pending?pending=${true}&page=${this.options.page}&size=${this.options.size}&search=${this.options.search}&orderBy=${this.options.orderBy}&orderDir=${this.options.orderDir}`;
  ...
  fetchAllOrders() {
    this.isLoadingResults = true;
    const urlString = `hos-cases/pending/?pending=${true}&page=${this.paginator.pageIndex}&size=${this.paginator.pageSize}&search=${this.options.search}&orderBy=${this.sort.active}&orderDir=${this.sort.direction}`;
  ...
  updateAnalysis(sample: LabSample) {
    // this.router.navigate(['/dashboard/updateanalysisorder', sample.id]);
    this.router.navigate(['/dashboard/updateanalysisorder'], { queryParams: { caseId: sample.id } });
  ...
  getData(): Observable<any> {
    this.isLoadingResults = true;
    const urlString = `hos-cases/?pending=${true}&page=${this.paginator.pageIndex}&size=${this.paginator.pageSize}&search=${this.options.search}&orderBy=${this.sort.active}&orderDir=${this.sort.direction}`;
    return this.repositoryService.getData(urlString).pipe(catchError(() => observableOf(null)));
  }
